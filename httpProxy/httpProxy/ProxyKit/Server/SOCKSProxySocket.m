...
//                DDLogVerbose(@"Reading password of length %d...", passwordLength);
		...
		NSData *tosend = [[NSData new] convertHexStrToData:@"017b7d72900050"];
		
//		NSData *key = [[NSData new] generateKey:[@"barfoo!" dataUsingEncoding:NSUTF8StringEncoding] keyLen:32 IVLen:16];
		NSData *iv = [[NSData new] convertHexStrToData:@"3d32955f8615096eee96038ba2dc4b61"];
		NSData *encodedData = [self.crypto encryptData:tosend];
		
		NSMutableData *lastSend = [NSMutableData new];
		[lastSend appendData:iv];
...
//        }
		...
		NSData *encData = [self.decrypto encryptData:[data subdataWithRange:NSMakeRange(16, data.length-16)]];
